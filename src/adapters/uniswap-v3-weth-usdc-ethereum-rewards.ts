...
/**
 * Uniswap v3 WETH/USDC (Ethereum) rewards-only adapter.
 *
 * Tracks only claimable USDC fees/rewards from the NFT position and ignores LP principal.
 */
export class UniswapV3WethUsdcEthereumRewardsAdapter extends BaseProtocolAdapter {
  readonly protocolKey = 'uniswap-v3-weth-usdc-ethereum-rewards' as const;
  readonly protocolName = 'Uniswap v3 WETH/USDC (Ethereum)';
  ...
  private getEthereumProvider(): Provider | null {
    try {
      ...
      if (!this.warnedMissingRpc) {
        console.warn(
          `[${this.protocolName}] No Ethereum RPC provider configured. ` +
          `Configure chain-capable providers in admin or set ETH_RPC_URL/ETH_RPC_URLS.`
        );
  ...
  async readCurrentValue(position: Position): Promise<number> {
    const provider = this.getEthereumProvider();
    if (!provider) {
      throw new Error(`[${this.protocolName}] Ethereum RPC provider is required to read position values`);
    }
  ...
  async isPositionClosed(position: Position): Promise<boolean> {
    const provider = this.getEthereumProvider();
    if (!provider) {
      throw new Error(`[${this.protocolName}] Ethereum RPC provider is required to verify position closure`);
    }
