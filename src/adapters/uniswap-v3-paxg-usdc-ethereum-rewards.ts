...
export class UniswapV3PaxgUsdcEthereumRewardsAdapter extends BaseProtocolAdapter {
  ...
  private getEthereumProvider(): Provider | null {
    ...
    if (!rpcUrl) {
      if (!this.warnedMissingRpc) {
        console.warn(`[${this.protocolName}] ETH_RPC_URL is not set. Skipping adapter.`);
        this.warnedMissingRpc = true;
      }
      return null;
    }

    this.ethereumProvider = new ethers.JsonRpcProvider(rpcUrl, ETHEREUM_CHAIN_ID);
    return this.ethereumProvider;
  }
  ...
  async readCurrentValue(position: Position): Promise<number> {
    const provider = this.getEthereumProvider();
    if (!provider) {
      throw new Error(`[${this.protocolName}] ETH_RPC_URL is required to read position values`);
    }
  ...
  async isPositionClosed(position: Position): Promise<boolean> {
    const provider = this.getEthereumProvider();
    if (!provider) {
      throw new Error(`[${this.protocolName}] ETH_RPC_URL is required to verify position closure`);
    }
